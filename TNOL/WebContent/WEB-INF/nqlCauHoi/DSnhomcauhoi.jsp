...
<head>
	...
	.ThemNQL1{position: fixed;left: -20px;font-size: 23px;border-radius: 7px;width:370px}
	.ThemNQL3{position: fixed;left: -270px;font-size: 23px;width:500px}
	#btnXoa{background-color:LightCoral;color:DarkSlateGrey;font-size:18px; position: relative;top: -15px;left: -20px;padding:5px 15px}
	#btnXoa:hover{background-color: IndianRed}
	#btnThem{background-color:#67BF7F;color:DarkSlateGrey;font-size:18px;position: relative;top: -15px;left: -30px;padding:5px 15px}
 		...
 		<div style="width:100%;height:1600px;position:fixed;top:400px">
			...
			<th><a href="logout"><u class="tb2" >ĐĂNG XUẤT</u></a></th>
 			...
 			<table class="table1" border="0px" cellpadding="15px">
		        ...
		        <tr><th  class="tbl1"><a href="nqlch_nhomCauHoiList" style="color:DarkBlue">DANH SÁCH NHÓM CÂU HỎI</a></th></tr><br>
  			...
  			<b style="font-size: 35px; color:DarkSlateGray">Danh sách nhóm câu hỏi</b>
				...
				<div style="margin-left:-20px;width:990px;height: 890px;background-color: #F7F8E0;border-radius: 10px">
					<table style="margin-left:10px;font-size: 20PX;width:970px">
					    <thead>
					      <tr>
					        <th style="text-align: center;width:300px;padding:7px;border-right:#000 solid 1px">Tên nhóm câu hỏi</th>
					        <th style="text-align: center;width:300px;padding:7px;border-right:#000 solid 1px">Tên môn học</th>
					        <th style="text-align: center;width:180px;padding:7px;border-right:#000 solid 1px">Chỉnh sửa</th>
					        <th style="text-align: center;padding:7px">Xóa</th>
					      </tr>
					    </thead>
					</table>
					    <div style="width:970px;height: 830px;overflow:auto">
					<table style="margin-left:10px;font-size: 20PX;width:970px" border="1px">
					    <tbody id="noidung">
					    	<c:forEach var="nch" items="${requestScope.listNhomCauHoi }">
							    <tr>
							        <td style="width:300px;padding:7px">${nch.getTenNhomCauHoi() }</td>
							        <c:forEach var="mh" items="${requestScope.listMonHoc }">
							        <c:if test="${nch.getMonHoc()==mh.getSubjectId() }">
							        <td style="width:300px;padding:7px;text-align: center">${mh.getSubjectName() }</td>
							        </c:if>
							        </c:forEach>
							        <td style="text-align: center;width:180px"><a href="nqlch_nhomCauHoi_edit?maNhomCauHoi=${nch.getMaNhomCauHoi()}&command=edit"><span class="glyphicon glyphicon-cog"></span></a></a></td>
							        <td style="text-align: center"><a href="nqlch_nhomCauHoi_edit?maNhomCauHoi=${nch.getMaNhomCauHoi()}&command=delete"><span class="glyphicon glyphicon-trash"></span></a></td>
							    </tr>
					      	</c:forEach>
					    </tbody>
				  </table>
...
<div class="container" style="position:relative;top:0px;left:0px" id="themUser" >
	...
	<button type="button" class="btn1 btn-info btn-lg" data-toggle="modal" data-target="#phucModal"><b>Thêm nhóm câu hỏi</b></button>
	<!-- Modal -->
	<div class="modal fade" id="phucModal" role="dialog" >
		<div class="modal-dialog" style="position: relative;top: 0px;left: 0px;width: 00px">
			<!-- Modal content-->
			<div class="modal-content" style="position: relative;top: 0px;left: -300px;width: 700px;height:360px;background-color:#F7F8E0">
				<div class="modal-header" style="position: relative;top: 0px;left: 0px;width: 695px">
					<button type="button" class="close" data-dismiss="modal" style="position: relative;top: 0px;left: 0px;z-index: 9999">&times;</button>
					<h4 class="modal-title" style="position: relative;top: 0px;left: 0px"><span class="glyphicon glyphicon-pencil"></span><b style="margin-left:12px">Thêm nhóm câu hỏi</b></h4>
				</div>
				<div class="modal-body" style="position: relative;top: 0px;left: 0px;width: 700px;height: 230px">
					<p>
					<br><br>
					<form action="nqlch_nhomCauHoi_edit" id="themNhomCauHoi">
						<b class="ThemNQL3">Tên nhóm câu hỏi:</b>
 						<input type="text" name="tenNhomCauHoi" class="ThemNQL1" required>
 						<br><br><br><br>
						<b class="ThemNQL3">Tên môn học:</b>
						<select name="monHoc" class="ThemNQL1" data-toggle="dropdown" style="border-radius: 5px;width:370px;height:37px" >
						<c:forEach var="MH" items="${requestScope.listMonHoc}">
				...
				<div class="modal-footer" style="position: relative;top: 0px;left: 0px;width: 695px;height:100px">
					<form action="nqlch_nhomCauHoiList">
					<button type="submit" class="btn" id="btnXoa"><span class="glyphicon glyphicon-remove" style="margin-top: 5px"></span><b style="margin-left:20px">Hủy</button>
					</form>
					<button type="submit" form="themNhomCauHoi" class="btn" id="btnThem" name="command" value="insert"><span class="glyphicon glyphicon-ok" style="margin-top: 5px"></span><b style="margin-left:5px"><b>Thêm</b></button>
				</div>
			</div>
		</div>
	</div>
