...
<head>
	...
	.ThemNQL1{position: fixed;left: 630px;font-size: 25px;border-radius: 7px;width:450px; color:Black}
	.ThemNQL3{position: fixed;left: 350px;font-size: 25px;width:500px;color:Black}
	#btnXoa{background-color: LightCoral;font-size:18px; position: relative;top: -67px;left: 460px;padding:5px 15px;width:130px}
	#btnXoa:hover{background-color: IndianRed}
	#btnThem{background-color: #67BF7F;color: DarkSlateGrey;font-size:18px;position:relative;top:-300x;left: 630px;padding:5px 15px;width:180px}
			...
			<th><a href="logout"><u class="tb2" >ĐĂNG XUẤT</u></a></th>
 			...
 			<table class="table1" border="0px" cellpadding="15px">
  			...
  			<div class="HienThongTin">
  				...
  				<form action="nqlch_nhomCauHoi_edit" method="post" id="edit" style="background-color: #e0e0e0;width:450px;height:320px;border-radius: 10px;margin-left:10px;margin-top:28px;padding:30px">
 					<input type="hidden" name="maNhomCauHoi" value="${requestScope.maNhomCauHoi }"/>
 					<b class="ThemNQL3">Tên nhóm câu hỏi:</b>
 					<c:forEach var="nch" items="${requestScope.listNhomCauHoi }">
 					<c:if test="${nch.getMaNhomCauHoi()== requestScope.maNhomCauHoi}">
					<input type="text" name="tenNhomCauHoi" class="ThemNQL1" style="background-color: #F7F8E0" value="${nch.getTenNhomCauHoi() }">
					</c:if>
					</c:forEach>
					<br><br><br><br>
					<b class="ThemNQL3">Tên nhóm câu hỏi:</b>
					<select name="monHoc" class="ThemNQL1" data-toggle="dropdown" style="border-radius: 5px;width:450px;height:37px;background-color: #F7F8E0" >
					<c:forEach var="MH" items="${requestScope.listMonHoc}">
					<c:if test="${MH.getSubjectId()==requestScope.nhomCauHoi.getMonHoc() }">
					...
					<button type="submit" class="btn" id="btnThem" name="command" value="edit" form="edit"><span class="glyphicon glyphicon-ok" style="margin-top: 5px"></span><b style="margin-left:5px"><b>Lưu thay đổi</b></button>
