...
<head>
	...
	width:1050px; height: 1100px;position:relative; left:285px;top:-287px;border-radius: 10px}
...
<div id="modal">
	...
	<th><a href="logout"><span class="tb2" >ĐĂNG XUẤT</span></a></th>
			...
			<table class="table1">
		        ...
		        <tr><th align ="left" class="tbl1"><a href="nqldt_deThiList"style="color:DarkBlue">DANH SÁCH ĐỀ THI</a></th></tr>
		        <tr><th align ="left" class="tbl1"><a href="nqldt_soanDeThi">SOẠN ĐỀ THI</a></th></tr>
 		...
 		<div class="HienThongTin">
 			<form action="nqldt_deThi_edit" method="post">
		 		<span class="glyphicon glyphicon-pencil" style="color:#000"></span><b style="margin-left:12px; color:#000">CHỈNH SỬA ĐỀ THI</b><br><br>
				<b style="color:#000">Tên đề thi:</b> 
				<input id="TieuDe" type="text" class="form-control" value="${requestScope.getDeThi.getTenDeThi()}" name="tenDeThi" placeholder="Tên đề thi" style="background-color:#F7F8E0;width: 600px;font-size:20px;color:#000"/><br>
				<b style="color:#000">Thời gian thi:</b> 
				<input id="TieuDe" type="number" class="form-control" value="${requestScope.getDeThi.getThoiGianLamBai()}" name="thoiGianThi" placeholder="Thời gian thi" style="border:#000 solid 1px;background-color:#F7F8E0;width: 200px;font-size:20px;color:#000"/><b style="position: relative;top:-35px;left:210px;font-size: 25px;color:#000">phút</b><br>
				<input type="hidden" name="nguoiTaoDeThi" value="${requestScope.getDeThi.getNguoiTaoDeThi()}"/>
				<b style="color:#000">Người tạo đề thi:</b> 
				<input id="TieuDe" type="text" class="form-control" value="${requestScope.usDal.viewInfo(getDeThi.getNguoiTaoDeThi()).getFullname() }" readonly style="background-color:#F7F8E0;width: 600px;font-size:20px;color:#000"/><br>
				<input type="hidden" name="maDeThi" value="${requestScope.maDeThi }"/>
				<b style="color:#000">Môn học:</b>
				<select name="color" data-toggle="dropdown" style="background-color:#F7F8E0;color:#000;width:250px;height:40px;border-radius: 5px" id="monHocChose">
				<c:forEach var="mh" items="${requestScope.listMonHoc }">
					<c:if test="${requestScope.maMonHoc==mh.getSubjectId() }">
				...
				<select name="color" data-toggle="dropdown" style="background-color:#F7F8E0;color:#000;height:40px;width:250px;border-radius: 5px" id="nhomCauHoiChose">
				</select>
				...
				<br><br><br>
				<input id="themDeThi" type="submit" class="button" name="them" value="Lưu thay đổi" style="width: 150px;height:45px; font-size:20px;background-color: LightBlue;color:Black;position: relative;;left: 650px;top:-45px;border-radius: 8px">
			</form>	
			<form action="nqldt_deThiList">
				<input id="huyThayDoi"type="submit"class="button"name="huy"value="Hủy thay đổi"style="position:relative;top:-89px;left:455px;width:150px;height:45px;font-size:20px;background-color:IndianRed	;color:Black;border-radius:8px">
   			 </form>
   			 <br><br>
  			  <br>
...
<script>
	<c:if test="${mapNhomCauHoi.size()>0}">
		$("#monHocChose").prop("disabled",true);
	</c:if>
	
	<c:forEach items="${requestScope.mapNhomCauHoi.keySet()}" var="entry">
		var f1=${entry.getMaNhomCauHoi()};
		<c:forEach var="k" items="${requestScope.listNhomCauHoi}">
			if(f1==${k.getMaNhomCauHoi()}){
				var div = "<div id ='"+ ${entry.getMaNhomCauHoi()}+"'>";
				var soLuongCauHoi="<input type='number' min='0' max='${chDal.soLuongCauHoiTheoMaNhomCauHoi(k.getMaNhomCauHoi())}' value='${mapNhomCauHoi.get(entry)}' name='soLuongCauHoi_"+${entry.getMaNhomCauHoi()}+"'class='soLuongCauHoi' placeholder='Nhập số lượng câu hỏi'/>";
				var xoa="<input type='button' onclick='deleteNCH("+${entry.getMaNhomCauHoi()}+")' class='button xoa' name='xoaNhomCauHoi' value='xoa'><br><br>";
				var tenNhomCauHoi="<span style='color:#000'>"+"${entry.getTenNhomCauHoi()}"+"</span>";
			}
		</c:forEach>
	...
	$("#themNhomCauHoi").click(function(){
		...
		<c:forEach var="k" items="${requestScope.listNhomCauHoi}">
			if(f==${k.getMaNhomCauHoi()}){
				var soLuongCauHoi="<input type='number' min='0' max='${chDal.soLuongCauHoiTheoMaNhomCauHoi(k.getMaNhomCauHoi())}' name='soLuongCauHoi_"+nhomCH.val()+"'class='soLuongCauHoi' placeholder='Nhập số lượng câu hỏi' style='background-color:#F7F8E0'/>";
				var xoa="<input type='button' onclick='deleteNCH("+nhomCH.val()+")' class='button xoa' name='xoaNhomCauHoi' value='xoa'><br><br>";
				var tenNhomCauHoi="<span >"+TNCH+"</span>";
			}
		</c:forEach>
		$("#themNCH").append(div + tenNhomCauHoi+":"+soLuongCauHoi+xoa + "</div>");
