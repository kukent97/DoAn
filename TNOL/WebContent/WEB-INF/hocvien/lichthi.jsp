 		...
 		<div class"than" style="width:100%;height:1600px;position:fixed;top:400px">
			...
			<th><a href="logout"><u class="tb2" >ĐĂNG XUẤT</u></a></th>
	 		...
	 		<table class="table1" border="0px" cellpadding="15px" >
  			...
  			<b style="font-size: 35px; color:DarkSlateGray">Xem lịch thi</b><br><br>
			<table class="table" border="2px" style="margin-left: -23px;font-size: 20PX;text-align:center">
			    <thead >
			      <tr>
			      	<th style="text-align:center">Kỳ thi</th>
			        <th style="text-align:center">Môn thi</th>
			        <th style="text-align:center">Ngày thi</th>
			        <th style="text-align:center">Thời gian thi</th>
			        <th style="text-align:center"></th>
			      </tr>
			    </thead>
			    <tbody>
			    <c:forEach var="lichThi" items="${requestScope.listExamFollowClass }">
			      <tr>
			      <td>${lichThi.getTenKyThi()}</td>
			      <c:forEach var="mh" items="${requestScope.listMonHoc }">
			      	<c:if test="${mh.getSubjectId()==mhDal.getMonHocTheoMaNhomCauHoi(text_questionDal.listTextQuestion(lichThi.getMaDeThi())[0].getNhomCauHoi())}">
			        <td>${mh.getSubjectName()}</td>
			        </c:if>
			        </c:forEach>
			        <td>${lichThi.getNgayThi() }</td>
			        <td>${requestScope.examDal.thoiGianThi(lichThi.getMaKyThi())} Phút</td>
			        <c:set var="daThi" value="0"></c:set>
			        <c:forEach var="list" items="${requestScope.listDiem }">
				        <c:if test="${list.getKyThi()!=lichThi.getMaKyThi() }">
				        	<c:set var="daThi" value="1"></c:set>
				      	</c:if>
			      	</c:forEach>
			      	<c:if test="${daThi==0}">
				        <td><a href="hocVien_batDauLamBai?thoiGian=${requestScope.examDal.thoiGianThi(lichThi.getMaKyThi())}&maKyThi=${lichThi.getMaKyThi() }&maDeThi=${lichThi.getMaDeThi()}&ngayThi=${lichThi.getNgayThi()}">Vào làm bài</a></td>
				    </c:if>
				    <c:if test="${daThi==1}">
				      	<td style="color:Brown"><u>Đã làm</u></td>
				    </c:if>
			      </tr>
			    </c:forEach>
			    </tbody>
