...
<head>
	...
	#btnXoa{background-color: MediumSeaGreen;font-size:20px; position: relative;top: 90px;left: 370px;padding:5px 15px;width:130px}
	#btnXoa:hover{background-color: IndianRed}
	#btnThem{background-color: MediumSeaGreen;font-size:20px;position: relative;top: 50px;left: 510px;padding:5px 15px;width:150px}
			...
			<th><a href="logout"><u class="tb2" >ĐĂNG XUẤT</u></a></th>
 			...
 			<table class="table1" border="0px" cellpadding="15px">
  			...
  			<div class="HienThongTin">
	  			<b style="font-size: 35px; color:DarkRed"><span class="glyphicon glyphicon-pencil"></span>Chỉnh sửa môn học</b>
	  			<form action="admin_monHoc_delete" method="post" id="Validate">
					<br><br>
					<c:forEach var="monHoc" items="${requestScope.listMonHoc }"><c:if test="${monHoc.getSubjectId()==maMonHoc}">
					<b class="ThemNQL3">Mã môn học:</b>
					<input type="text" name="maMonHoc" style="background-color: LightSlateGray" class="ThemNQL1" value="${monHoc.getSubjectId()}"readonly>
					<br><br><br>
					<b class="ThemNQL3">Tên môn học:</b>
					<input type="text" name="tenMonHoc" class="ThemNQL1" value="${monHoc.getSubjectName() }">
					<br><br><br>
 					</c:if>
					</c:forEach>
				</form>	
				<form action="monHocList">
					<button type="submit" class="btn" id="btnXoa" onclick="getBack()">Hủy</a></button>
				</form>
					<button type="submit" class="btn" id="btnThem" value="edit" name="command" form="Validate">Lưu thay đổi</button></form>
			</div>
